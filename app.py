...
recaptcha_Secret_key = "6LeN-wQpAAAAAM3vNlGTQPO9uvODtv1BrcADvppM"
...
db_config = {
    ...
    'user': 'root',
    'db': 'bug_hound',
    'password': 'Noone@stark1',
    'charset': 'utf8mb4',
    'cursorclass': pymysql.cursors.DictCursor
}


def delete_old_soft_deleted_bugs():
    seven_days_ago = datetime.now() - timedelta(days=7)
    connection = pymysql.connect(**db_config)
    try:
        with connection.cursor() as cursor:
            # SQL statement to delete bugs soft deleted more than 7 days ago
            sql = "DELETE FROM bug WHERE is_deleted = 1 AND soft_delete_date <= %s"
            print(sql)
            cursor.execute(sql, (seven_days_ago,))
            connection.commit()
    finally:
        connection.close()
    print("Deleted old soft-deleted bugs.")


# Initialize Scheduler
scheduler = BackgroundScheduler()
scheduler.add_job(func=delete_old_soft_deleted_bugs,
                  trigger="interval", days=1)
scheduler.start()


@app.teardown_appcontext
def shutdown_scheduler(sender, *args, **kwargs):
    if scheduler.running:  # Only shutdown if the scheduler is running
        scheduler.shutdown()
...
def insert():
    ...
    if request.method == "POST":
        ...
        connection = pymysql.connect(**db_config)
        with connection.cursor() as cursor:
            cursor.execute("INSERT INTO employees (name, username, password, userlevel) VALUES (%s, %s, %s, %s)",
...
def edit():
    ...
    if request.method == "POST":
        ...
        connection = pymysql.connect(**db_config)
        with connection.cursor() as cursor:
            cursor.execute("UPDATE employees SET name=%s, username=%s, password=%s, userlevel=%s WHERE emp_id=%s",
                           (name, username, password, userlevel, emp_id))
            connection.commit()
...
def delete(id_data):
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # Update the DELETE statement to an UPDATE statement
        cursor.execute(
            "UPDATE employees SET is_deleted = 1, deleted_at = NOW() WHERE emp_id = %s", (id_data,))

        connection.commit()
        message = f"Employee with id {id_data} was successfully marked as deleted."
...
def add_program():
    ...
    if request.method == "POST":
        ...
        connection = pymysql.connect(**db_config)
        with connection.cursor() as cursor:
            cursor.execute("INSERT INTO programs (program,program_release,program_version) VALUES (%s, %s, %s)",
...
def edit_program():
    ...
    if request.method == "POST":
        ...
        connection = pymysql.connect(**db_config)
        with connection.cursor() as cursor:
            cursor.execute("UPDATE programs SET program=%s, program_release=%s, program_version=%s WHERE prog_id=%s",
                           (program, program_release, program_version, prog_id))
            connection.commit()
...
def edit_area():
    ...
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE areas SET area=%s, prog_id=%s WHERE area_id=%s", (area, prog_id, area_id))
            connection.commit()
...
def update_bug():
    ...
    report_types = ['coding error', 'design error',
                    'hardware error', 'suggestion', 'Documentation', 'Query']
    ...
    resolution = ['Pending', 'Fixed', 'Irreproducible', 'Deferred', 'As designed',
...
def show_recovered_bugs():
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # The modified query to select only deleted bugs
    ...
    return render_template('update_bug.html', username=username, userlevel=userlevel,
                           bugs=data, programs=programs, report_types=report_types, severities=severities,
                           employees=employees, areas=areas, resolution=resolution, resolution_version=resolution_version,
                           priority=priority, status=status, showing_recovered=True)
...
def delete_bug(id_data):
    ...
    with connection.cursor() as cursor:
        # Update the DELETE statement to an UPDATE statement
        cursor.execute(
            "UPDATE bug SET is_deleted = 1, deleted_at = NOW() WHERE bug_id = %s", (id_data,))
...
def edit_bug():
    ...
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT prog_id FROM programs WHERE program=%s", (program,))
            prog_id = cursor.fetchone()

            connection.commit()

            cursor.execute(
                "SELECT area_id FROM areas WHERE area=%s", (functional_area,))
...
def view_attachment(filename):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT attachment FROM bug WHERE filename = %s", (filename,))
...
def export_data():
    ...
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            ...
            if data_type == "xml":
                tree.write(f"{table_name}.xml", encoding="utf-8",
...
# recover bug functionality ( based on soft delete)
...
def recover_bug(id_data):
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # Update the bug record to set is_deleted to 0 and deleted_at to NULL
        cursor.execute(
            "UPDATE bug SET is_deleted = 0, deleted_at = NULL WHERE bug_id = %s", (id_data,))

        connection.commit()
        message = f"Bug with id {id_data} has been successfully recovered."

        flash(message=message)
        return redirect(url_for('update_bug'))

# delete all bugs at a time


@app.route('/delete_permanently/<int:id_data>', methods=['POST', 'GET'])
def delete_permanently(id_data):
    # Ensure the user is logged in and authorized to delete bugs
    if "loggedin" not in session:
    ...
    try:
        with connection.cursor() as cursor:
            # Execute the SQL command to delete the bug
            cursor.execute('DELETE FROM bug WHERE bug_id = %s', (id_data,))
            connection.commit()
            flash('Bug deleted permanently.', 'success')
    except Exception as e:
        flash('An error occurred while trying to delete the bug.', 'danger')
        print(e)  # For debugging purposes, print the error to the console
    finally:
        connection.close()

    # Redirect to the page showing recovered bugs
    return redirect(url_for('show_recovered_bugs'))


# recover all bugs at a time
...
def recover_all_bugs():
    ...
    try:
        with connection.cursor() as cursor:
            # Execute the SQL command to recover all bugs
            cursor.execute('UPDATE bug SET is_deleted=0 WHERE is_deleted=1')
            connection.commit()
            flash('All bugs recovered successfully.', 'success')
    except Exception as e:
        flash('An error occurred while trying to recover all bugs.', 'danger')
        print(e)
    finally:
        connection.close()
...
# delete all bugs permanently -- hard delete
...
def delete_all_bugs():
    ...
    try:
        with connection.cursor() as cursor:
            # Execute the SQL command to delete all bugs
            cursor.execute('DELETE FROM bug WHERE is_deleted=1')
            connection.commit()
            flash('All bugs deleted permanently.', 'success')
    except Exception as e:
        flash('An error occurred while trying to delete all bugs.', 'danger')
        print(e)
    finally:
        connection.close()
...
def submit_form():
    ...
    success, error = verify_recaptcha(recaptcha_response)
